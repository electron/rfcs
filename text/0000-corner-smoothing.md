...
A simple strategy for making round corners is to replace each corner with a quarter circle. This is the strategy [used by the Web for `border-radius`](https://www.w3.org/TR/css-backgrounds-3/#border-radius). More rounding strategies exist to satisfy additional visual and mathematical design constraints.

Apple is well-known for having smooth corners in their hardware and software product designs. Apple's macOS is one of Electron's major supported platforms, and round corners are a core part of its design language, used across UI elements such as buttons and windows. The round corners used in macOS are a different shape than those created by the Web's `border-radius` rule.

Integrating with the operating system and its design language is important to many desktop applications. For example, Chromium emulates [macOS's scrollbar design](https://source.chromium.org/chromium/chromium/src/+/main:ui/views/controls/scrollbar/cocoa_scroll_bar.h?q=symbol%3A%5Cbviews%3A%3ACocoaScrollBar%5Cb%20case%3Ayes). Like scroll bars, the shape of a rounded corner can be a subtle detail to many users, but making it align closely to the system's design language that users are familiar with makes the application's design feel familiar too.
...
Corner smoothing is controlled using the `-electron-corner-smoothing` CSS rule. This rule works on any element that has the `border-radius` rule applied to it.
...
The `-electron-corner-smoothing` CSS rule controls how far the corner's curvature extends into the edge of the element. It is a percentage between `0%` (default) and `100%`. To match the system rounding in macOS, use a value of `60%`.

![](../images/0000/Values.svg)

This CSS rule is available across all platforms. This CSS rule also affects the shape of borders and shadows on the same element.
...
At a high level, we're taking the points where the sides connect to the corner circle and pulling them back into the side. Thus, instead of constructing a round corner with a quarter circle, we use two cubic Bézier curves to construct the corner.

![](../images/0000/Construction.svg)

Reading the original article will give you an intuition for how the curve's control points are derived.
...
Applying smoothing to an existing round corner should retain as much of the original shape's key points and properties as possible. The smoothing procedure outlined above satisfies a few important mathematical constraints:
...
Electron rarely makes changes to Blink. The few exceptions include exposing the `<webview>` element, exposing the `-webkit-app-region` CSS rule, and increasing the threshold for Web Storage APIs. With this feature, we would have to maintain a moderately-sized patch to Blink.
...
Implementing this feature within Blink uniquely offers the best performance and simplest developer experience in this problem space. Other potential solutions and avenues for development suffer from technical complexity, performance pitfalls, or aim to solve larger and more difficult problems than most people are asking for.

While the proposed solution here implies a maintenance burden, the areas of Blink touched by this patch change rarely.
...
Electron has dabbled in having its "own" CSS rules before. `-webkit-app-region` is a CSS rule that allows developers to define what regions in their application can drag the window. This CSS rule has been [proposed for standardization](https://github.com/w3c/csswg-drafts/issues/7017) as part of the proposed Window Controls Overlay feature.
...
- Other shapes: There may be desire from developers and designers for other shapes with smooth round corners, like the superellipse.
- Web standards: A future Web standard may one day provide this functionality. We should provide developers with a clear migration path to the standardized feature.
